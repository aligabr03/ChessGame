...
//TODO: Une fonction pour ajouter un Film à une ListeFilms, le film existant déjà; on veut uniquement ajouter le pointeur vers le film existant.  Cette fonction doit doubler la taille du tableau alloué, avec au minimum un élément, dans le cas où la capacité est insuffisante pour ajouter l'élément (contrairement au TD5 qui faisait simplement ne pas ajouter l'élément).  Il faut alors allouer un nouveau tableau plus grand, copier ce qu'il y avait dans l'ancien, et éliminer l'ancien trop petit.  Cette fonction ne doit copier aucun Film ni Acteur, elle doit copier uniquement des pointeurs.
//[
void changeDimension(ListeFilms& liste, int nouvelleCapacite)
{
	Film** nouvelleListe = new Film*[nouvelleCapacite];
	
	if (liste.elements != nullptr) {  // Noter que ce test n'est pas nécessaire puique nElements sera zéro si elements est nul, donc la boucle ne tentera pas de faire de copie, et on a le droit de faire delete sur un pointeur nul (ça ne fait rien).
		liste.nElements = min(nouvelleCapacite, liste.nElements);
...
void ajouterFilm(ListeFilms& liste, Film* film)
{
	if (liste.nElements == liste.capacite)
		changeDimension(liste, max(1, liste.capacite * 2));
...
span<Film*> spanListeFilms(const ListeFilms& liste) { return span(liste.elements, liste.nElements); }

void enleverFilm(ListeFilms& listeFilms, const Film* film)
{
	for (Film*& element : spanListeFilms(listeFilms)) {  // Doit être une référence au pointeur pour pouvoir le modifier.
		if (element == film) {
			if (listeFilms.nElements > 1)
				element = listeFilms.elements[listeFilms.nElements - 1];
...
)
{
	...
	*/
	...
	//[
		ajouterFilm(acteur->joueDans, filmp);
	//]
	}
	//[
	return filmp;
...
ListeFilms creerListe(string nomFichier)
{
	...
	*/
	for (int i : range(nElements)) { //NOTE: On ne peut pas faire un span simple avec spanListeFilms car la liste est vide et on ajoute des éléments à mesure.
		listeFilms.nElements = i; //NOTE: On doit s'assurer que nFilms est bon avant l'appel à lireFilm, pour que la recherche des acteurs déjà présents fonctionne.  Comme on a initialisé à 0, on pourrait aussi faire ++ après l'ajout du film.
		ajouterFilm(listeFilms,
		//]
		lireFilm(fichier//[
		, listeFilms
		//]
...
{
	return acteur->joueDans.nElements != 0;
}
...
void afficherFilmographieActeur(const ListeFilms& listeFilms, const string& nomActeur)
{
	...
	const Acteur* acteur = //[
		trouverActeur(listeFilms, nomActeur);
		/* //]
		nullptr;
	//[ */
	//]
...
int main()
{
	...
	enleverFilm(listeFilms, nullptr); // Enlever un film qui n'est pas dans la liste (clairement que nullptr n'y est pas).
